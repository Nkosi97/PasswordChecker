...
let passwordIsOkay =  myTest.passwordIsOkay()
let password_is_valid = myTest.password_is_valid()

//password_is_valid
describe("password is valid function", () => {
  it(`should return password is Valid`, () => {
    expect(password_is_valid("")).toBe(`password shoud exist`)
  });
  it(`should return password is Valid`, () => {
    expect(password_is_valid("2")).toBe(`password shoud have eight characters`)
  });
  it(`should return password is Valid`, () => {
    expect(password_is_valid("lkjuhgyf")).toBe(`password shoud atleast have one upper character`)
  });
  it(`should return password is Valid`, () => {
    expect(password_is_valid("l*dobeatS")).toBe(`password shoud atleast have one special character`)
  });
  it(`should return password is Valid`, () => {
    expect(password_is_valid("Lavis@97")).toBe(`password is valid!`)
  });
